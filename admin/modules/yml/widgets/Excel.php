...
namespace admin\modules\yml\widgets;
...
class Excel extends \yii\base\Widget {
     ...
     */
    public $formatter;
    public $styleAllBorders = ['borders' =>
        [
        ...
        foreach ($models as $model) {
            ...
            if ($this->setFirstTitle && !$hasHeader) {
                ...
                foreach ($columns as $key => $column) {
                    ...
                    if ($column['width']) {
                        $activeSheet->getColumnDimension($col)->setWidth($column['width']);
                    }
                    $colnum++;

                    $activeSheet->getStyle($col . '1')->applyFromArray(array_merge(
                                    [
                        'font' => ['size' => 9],
                        'alignment' => [
            ...
            foreach ($columns as $key => $column) {
                ...
                if ($column['format'] == 'text') {
                    $activeSheet->getCell($col . $row)->setValueExplicit($column_value, \PHPExcel_Cell_DataType::TYPE_STRING);
                } else {
                    $activeSheet->setCellValue($col . $row, $column_value);
                }
                if ($column['color']) {
                    $activeSheet->getStyle($col . $row)->applyFromArray(array_merge(['fill' =>
                        [
                            'type' => \PHPExcel_Style_Fill::FILL_SOLID,
                            'startcolor' =>
                            [
    ...
    public function executeGetColumnData($model, $params = []) {
        ...
        if (isset($params['value']) && $params['value'] !== null) {
            ...
            if (array_key_exists($params['attribute'], $model->data)) {
                $value = $model->data[$params['attribute']];
            } else {
