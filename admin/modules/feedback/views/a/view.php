...
<dl class="dl-horizontal feedback-view">
    ...
    <?php if ($model->type == Feedback::TYPE_CALLBACK) {
        ?>
    ...
    <dd><?= $model->name ?></dd>
    <?php if ($model->type == Feedback::TYPE_FEEDBACK) { ?>
        <dt>E-mail</dt>
        <dd><?= $model->email ?></dd>
    <?php } ?>
    <?php if ($this->context->module->settings['enablePhone'] || $model->type == Feedback::TYPE_CALLBACK) {
        ?>
        <dt><?= Yii::t('admin', 'Телефон') ?></dt>
        <dd><?= $model->phone ?></dd>
    <?php } ?>
    <?php if ($this->context->module->settings['enableTitle'] && $model->type == Feedback::TYPE_FEEDBACK) : ?>
