<?php
namespace admin\modules\sale\controllers;
...
class AController extends \admin\base\admin\Controller
{
            ...
            if(Yii::$app->request->isAjax){
                ...
                if(isset($_FILES) && $this->module->settings['enableThumb']){
                    ...
                    if($model->image && $model->validate(['image'])){
                        $model->image = Image::upload($model->image, 'sale');
                    }
                    else{
                        $model->image = '';
                    }
                ...
                if($model->save()){
                    $this->flash('success', Yii::t('admin/sale', 'Акция создана'));
    ...
    public function actionEdit($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if(Yii::$app->request->isAjax){
                ...
                if($model->save()){
                    $this->flash('success', Yii::t('admin/sale', 'Акция обновлена'));
                }
                else{
    ...
    public function actionDelete($id)
    {
        ...
        return $this->formatResponse(Yii::t('admin/sale', 'Акция удалена'));
    }
    ...
    {
        return $this->changeStatus($id, Sale::STATUS_ON);
    }

    public function actionOff($id)
    {
        return $this->changeStatus($id, Sale::STATUS_OFF);
    }
}